...
namespace EDSL_API.Controllers
{
    public class DivisionsController : ApiController
    {
        ...
        public IHttpActionResult PostDivision(Division division)
        {
            ...
            {
                if (DivisionExists(division.divCode))
                {
                    return Conflict();
                }
                else
            ...
            return CreatedAtRoute("DefaultApi", new { id = division.divCode }, division);
        }
        ...
        {
            return db.Divisions.Count(e => e.divCode == id) > 0;
        }
    }
}