...
namespace EDSL_API.Controllers
{
    public class RoundsController : ApiController
    {
        ...
        public IHttpActionResult PostRound(Round round)
        {
            ...
            {
                if (RoundExists(round.roundCode))
                {
                    return Conflict();
                }
                else
            ...
            return CreatedAtRoute("DefaultApi", new { id = round.roundCode }, round);
        }
        ...
        {
            return db.Rounds.Count(e => e.roundCode == id) > 0;
        }
    }
}